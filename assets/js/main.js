...
// JAM SISTEM REAL-TIME (LIVE CLOCK)
// Berjalan independen dari data MQTT -- selalu berdetak tiap detik lewat
// jam lokal browser, berbeda dengan "Terakhir Diperbarui" yang hanya
// berubah saat ada payload baru masuk dari perangkat.
...
window.parseIncomingJSON = function (payload) {
    ...
    try {
        ...
        if (payload.moisture !== undefined) setText('val-moisture', parseInt(payload.moisture));
        ...
        if (window.updateBadgeStatus) {
            if (payload.ph !== undefined) window.updateBadgeStatus('ph', payload.ph, payload.ph < 6.0, payload.ph > 6.5);
            if (payload.tds !== undefined) window.updateBadgeStatus('tds', payload.tds, payload.tds < 1260, payload.tds > 1540);
            if (payload.temp !== undefined) window.updateBadgeStatus('temp', payload.temp, payload.temp < 25, payload.temp > 32);
            ...
            if (payload.moisture !== undefined) window.updateBadgeStatus('moisture', payload.moisture, payload.moisture < 60, payload.moisture > 80);
        }
...
// ACTUATOR MONITOR
// Payload memakai key singkat (phup, phdown, nut1, nut2, valve1, valve2,
// vitamin, pwm) sesuai 7 channel relay pada block diagram v3:
// 4 dosing (pH+Nutrisi) + 2 valve + 1 sirkulasi vitamin (pompa diafragma).
window.updateActuatorPanel = function (payload) {
    if (!payload) return;

    ['phup', 'phdown', 'nut1', 'nut2', 'valve1', 'valve2', 'vitamin'].forEach((key) => {
